<template>
  <div class="mx-auto max-w-5xl">
    ...
    <div class="mb-5 rounded-2xl border border-blue-500/15 bg-blue-500/[.06] px-4 py-3 text-xs leading-5 text-blue-700 dark:text-blue-200">
      选择你想接收提醒的方式即可。飞书填写工作邮箱或手机号，不需要查找 OpenID。
    </div>
    ...
    <div v-else class="grid gap-4 md:grid-cols-2">
      <article v-for="channel in channels" :key="channel.channel" class="channel-card">
        ...
        <div v-else-if="!channel.configured && channel.channel === 'email' && authStore.isAdmin" class="provider-form"><p class="text-sm font-extrabold">开通邮件提醒</p><p class="provider-help">选择你的邮箱服务商，只填写邮箱地址与授权码。</p><button class="bind-button mt-4 w-full" @click="openEmailSetup">配置邮件服务</button></div>
        ...
        <form v-else class="mt-5" @submit.prevent="bind(channel)">
          ...
          <p v-if="channel.channel === 'feishu'" class="mt-2 text-[10px] leading-4 text-muted-foreground">填写企业工作邮箱或中国大陆手机号即可。</p>
        </form>
...
function inputLabel(channel: ReminderChannel) { return ({ email: '邮箱地址', sms: '中国大陆手机号', feishu: '飞书工作邮箱或手机号', qq: 'QQ 用户 OpenID' } as Record<string, string>)[channel] || '接收目标' }
function placeholder(channel: ReminderChannel) { return ({ email: 'name@example.com', sms: '13800138000', feishu: 'name@company.com 或 13800138000', qq: '机器人单聊事件中的 user_openid' } as Record<string, string>)[channel] || '' }
...
.channel-icon.inapp { @apply bg-blue-500/10 text-blue-500; }.channel-icon.email { @apply bg-violet-500/10 text-violet-500; }.channel-icon.sms { @apply bg-amber-500/10 text-amber-500; }.channel-icon.feishu { @apply bg-cyan-500/10 text-cyan-500; }.channel-icon.qq { @apply bg-rose-500/10 text-rose-500; }
