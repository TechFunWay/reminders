<template>
  <div class="mx-auto max-w-4xl">
    <div class="mb-4 flex items-end justify-between sm:mb-7">
      <div class="min-w-0">
        <p class="mb-1 text-xs font-extrabold uppercase tracking-[.18em] text-brand-500 sm:mb-2">消息与动态</p>
        <h1 class="font-display text-2xl font-extrabold tracking-tight sm:text-3xl">通知中心</h1>
        <p class="mt-0.5 text-[13px] text-muted-foreground sm:mt-2 sm:text-sm">{{ unread }} 条未读通知</p>
      </div>
      <button v-if="unread" class="shrink-0 rounded-xl border border-border bg-surface px-3 py-1.5 text-xs font-bold text-foreground transition hover:bg-muted sm:px-4 sm:py-2" @click="readAll">全部标为已读</button>
    </div>
    ...
    <div v-else-if="errorMsg" class="flex min-h-[300px] flex-col items-center justify-center gap-3 rounded-[28px] border border-dashed border-rose-500/30 bg-rose-500/[.04] px-6 text-center sm:min-h-[420px]">
      <svg viewBox="0 0 24 24" class="h-8 w-8 text-rose-500/70" fill="none" stroke="currentColor"><path d="M12 9v4m0 4h.01M10.3 3.8 2.5 17.2A2 2 0 0 0 4.2 20h15.6a2 2 0 0 0 1.7-2.8L13.7 3.8a2 2 0 0 0-3.4 0Z" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg>
      <h2 class="text-base font-extrabold text-rose-500">{{ errorMsg }}</h2>
    ...
    <div v-else-if="notifications.length" class="overflow-hidden rounded-[24px] border border-border/70 bg-surface/85 shadow-card backdrop-blur-xl">
      ...
      >
        <span class="notification-icon" :class="item.type === 'channel_failed' ? 'failure' : 'due'">
          ...
          <svg v-else viewBox="0 0 24 24" class="h-5 w-5" fill="none" stroke="currentColor"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9ZM10 21h4" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>
        </span>
        <span class="min-w-0 flex-1 text-left">
          ...
          <span class="mt-0.5 block text-xs leading-5 text-muted-foreground sm:mt-1">{{ item.body }}</span>
          <span class="mt-1 block text-[11px] font-medium text-muted-foreground/70 sm:mt-2">{{ formatTime(item.created_at) }}</span>
        </span>
    ...
    <div v-else class="flex min-h-[300px] flex-col items-center justify-center rounded-[28px] border border-dashed border-border bg-surface/35 text-center sm:min-h-[420px]">
      <span class="flex h-16 w-16 items-center justify-center rounded-[20px] bg-brand-500/10 text-brand-500 sm:h-20 sm:w-20 sm:rounded-[24px]">
        <svg viewBox="0 0 24 24" class="h-8 w-8 sm:h-9 sm:w-9" fill="none" stroke="currentColor"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9ZM10 21h4" stroke-width="1.6" stroke-linecap="round"/></svg>
      </span>
      <h2 class="mt-4 text-lg font-extrabold sm:mt-5">这里很安静</h2>
...
async function load() {
  ...
  try {
    ...
    // 失败不再伪装成「这里很安静」：区分「真的没有通知」和「接口失败」。
    notifications.value = []
    errorMsg.value = err.response?.data?.message || '读取通知失败，请重试'
  } finally {
    loading.value = false
  }
}
...
<style scoped>
.notification-row { @apply relative flex w-full items-center gap-3 border-b border-border/60 px-4 py-3 transition last:border-b-0 hover:bg-muted/50 sm:gap-4 sm:px-5 sm:py-5; }
...
.notification-icon { @apply flex h-9 w-9 shrink-0 items-center justify-center rounded-xl sm:h-11 sm:w-11 sm:rounded-2xl; }
